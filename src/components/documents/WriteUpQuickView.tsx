    ...
    <>
      ...
      <div className="fixed right-0 top-0 h-full w-[520px] bg-white shadow-xl z-50 flex flex-col animate-in slide-in-from-right duration-200">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border-light flex-shrink-0">
          ...
          <button
            onClick={onClose}
            className="w-8 h-8 flex items-center justify-center rounded-md text-text-secondary hover:bg-gray-50 transition-colors flex-shrink-0"
          >
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
          ...
          <div className={`flex items-center gap-2.5 px-4 py-3 rounded-lg border ${sc.bgColor} ${sc.borderColor}`}>
            ...
            <span className={`text-body font-medium ${sc.color}`}>{sc.label}</span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <p className="text-caption text-text-secondary">Employee</p>
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-body font-medium text-text-primary">{writeUp.location}</p>
              </div>
            </div>
            <div className="flex items-center gap-2.5">
              <Calendar className="w-4 h-4 text-text-secondary flex-shrink-0" />
              <div>
                <p className="text-caption text-text-secondary">Incident Date</p>
                <p className="text-body font-medium text-text-primary">{formatDate(writeUp.incidentDate)}</p>
              </div>
            </div>
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-body font-medium text-text-primary">{typeLabels[writeUp.type]}</p>
              </div>
            </div>
          </div>
          ...
          {writeUp.points !== undefined && (
            <div className="flex items-center gap-2 px-3 py-2 bg-gray-50 rounded-lg border border-border-light">
              <span className="text-caption text-text-secondary">Demerit Points:</span>
              <span className="text-body font-bold text-text-primary">{writeUp.points}</span>
            </div>
          )}

          {/* Description */}
          <div>
            <h3 className="text-callout text-text-secondary uppercase tracking-wider mb-2">
              Description
            </h3>
            <p className="text-body text-text-primary leading-relaxed bg-gray-50 rounded-lg border border-border-light px-4 py-3">
              {writeUp.description}
            </p>
          </div>
          ...
          {writeUp.employeeResponse && (
            <div>
              ...
              <p className="text-body text-text-primary leading-relaxed bg-blue-50 rounded-lg border border-blue-200 px-4 py-3">
                {writeUp.employeeResponse}
              </p>
            </div>
          )}
          ...
          {/* Signatures */}
          <div>
            <h3 className="text-callout text-text-secondary uppercase tracking-wider mb-3">
              Signatures
            </h3>
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between px-4 py-3 bg-green-50 rounded-lg border border-green-200">
                <div>
                  <p className="text-caption text-green-700 font-medium">Manager</p>
                  <p className="text-body text-text-primary">{writeUp.managerSignature.signedBy}</p>
                </div>
                <div className="flex items-center gap-1.5">
                  <CheckCircle2 className="w-3.5 h-3.5 text-green-600" />
                  <span className="text-caption text-green-700">{formatDateTime(writeUp.managerSignature.signedAt)}</span>
                </div>
              </div>
              ...
              {writeUp.employeeSignature ? (
                ...
                >
                  <div>
                    ...
                    <p className="text-body text-text-primary">{writeUp.employeeSignature.signedBy}</p>
                    {writeUp.employeeSignature.declineReason && (
                      <p className="text-caption text-red-600 mt-1 italic">
                ...
                <div className="flex items-center justify-between px-4 py-3 bg-gray-50 rounded-lg border border-border-light">
                  <div>
                    <p className="text-caption text-text-secondary font-medium">Employee</p>
                    <p className="text-body text-text-secondary italic">Awaiting signature</p>
                  </div>
              ...
              {[
                ...
                <div key={i} className="flex items-start gap-3 py-2 relative">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-body text-text-primary">{event.label}</p>
                    <p className="text-caption text-text-secondary">
